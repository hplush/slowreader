...
Run tests to be sure that everything is OK (open issue if tests don’t work in your environment):
...
It will run a web server and proxy (the website, by default, can make HTTP requests only to its own servers; we need a proxy to send HTTP requests to RSS feeds).
...
If you write docs, we recommend installing [LanguageTool](https://dev.languagetool.org/software-that-supports-languagetool-as-a-plug-in-or-add-on.html) for grammar check. For instance, for VS Code, [`davidlday.languagetool-linter`](https://marketplace.visualstudio.com/items?itemName=davidlday.languagetool-linter).
...
If you don’t want just to fix a specific bug but want to join the core team, here is the path:
...
3. After the onboarding, you can take tasks from the `Ready to Take` column. See the discussions there.
