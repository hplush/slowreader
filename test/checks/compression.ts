...
export async function checkCompression(pages: string[]): Promise<boolean> {
  ...
  for (let url of pages) {
    ...
    if (used !== 'br' && used !== 'zstd') {
      results.push(
        fail(`${url} has ${used ?? 'no'} compression instead of zstd`)
      )
    } else if (!response.headers.get('vary')?.includes('Accept-Encoding')) {
      results.push(fail(`${url} has no Vary: Accept-Encoding for caches`))
    } else {
      results.push(pass(`${url} is compressed by ${used}`))
    }
  }
